...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('.add-item').addEventListener('click', function () {
        ...
        const newRow = `
            <div class="row mb-3">
                <div class="col-md-6">
                    <select name="items[${counter}][product_id]" class="form-control" required>
                        <option value="" disabled selected>Select Product</option>
                        @foreach ($products as $product)
                        <option value="{{ $product->id }}">{{ $product->name }}</option>
                        @endforeach
                ...
                <div class="col-md-3">
                    <input type="number" name="items[${counter}][price]" class="form-control" placeholder="Price" required>
                </div>
                <div class="col-md-3">
                    <input type="number" name="items[${counter}][quantity]" class="form-control" placeholder="Quantity" required>
                </div>
                <div class="col-md-3">
